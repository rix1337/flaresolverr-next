...
| documentStartJs     | Optional, default none. JavaScript registered before navigation and run at document start on the requested page and every redirect document. Internal `data:` form pages used for POST requests are excluded. Registration is removed after the request so retained sessions do not carry it forward. Successful registration returns `"installed"` in `documentStartJsResult`.                                                                                                                           |
